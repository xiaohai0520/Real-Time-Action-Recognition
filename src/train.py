...
def train_model(dataset=dataset, save_dir=save_dir, num_classes=num_classes, lr=lr,num_epochs=nEpochs):
    ...
    train_params = [{'params': C3D.get_1x_lr_params(model), 'lr': lr},
                    {'params': C3D.get_10x_lr_params(model), 'lr': lr * 10}]
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val':
                torch.save(model,'{}model_epoch{}_accuracy_{}.pkl'.format(path,epoch+1,epoch_acc))
...
def draw_plot(train_list,dev_list,acc=True):
    plt.figure()
...
if __name__ == "__main__":
    ...
    draw_plot(train_acc_list,val_acc_list)
    draw_plot(train_loss_list,val_loss_list,False)